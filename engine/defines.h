...
// header from: https://github.com/dandersch/platform.h
/*
 ...
 * - SAME_TYPE(a,b): checks if two variables are the same type (ignoring
 * qualifiers)
 * - container_of(ptr, type, member): portable linux kernel-style
 * container_of(). Gets a pointer to the struct containing ptr as a member.
...
#if defined(__GNUC__)
#define COMPILER_GCC
#ifdef __clang__ // NOTE clang also defines __GNUC__ since it implements the gcc
                 // extensions
...
// NOTE GCC exports every symbol to the ELF by default, unless
// -fvisibility=hidden is specified
...
// #define CDECL __attribute__((__cdecl__)) // NOTE this define can cause
// warnings
#define CDECL
#elif defined(COMPILER_MINGW)
...
#define COMPILER_ENUM(e, ...)                                                  \
  COMPILER_TYPE_##e, // NOTE: _TYPE_ to not clash with the (more important)
                     // macro
typedef enum compiler_e { COMPILERS(COMPILER_ENUM) } compiler_e;
...
#define OPERATING_SYSTEM_STRING(e, str, ...)                                   \
  case OPERATING_SYSTEM_##e: {                                                 \
    string = str;                                                              \
  } break;
...
#define COMPILER_STRING(e, str, ...)                                           \
  case COMPILER_TYPE_##e: {                                                    \
    string = str;                                                              \
  } break;
...
// NOTE: to not show "expanded from macro.." in gcc: -ftrack-macro-expansion=0
...
// here a list of ways to implement a DEBUG_BREAK macro and on what OS+compiler
// they work see https://github.com/scottt/debugbreak for more info
//   __builtin_debugtrap();              linux: clang(++) windows: clang-cl.exe
//   clang(++)
//   __builtin_trap();                   linux: clang(++) gcc(++) mingw(++)
//   windows: clang(++) clang-cl.exe #include <signal.h> raise(SIGTRAP); linux:
//   clang(++) gcc(++) mingw(++) windows: none DebugBreak() linux: none windows:
//   cl.exe clang-cl.exe
//   __debugbreak()                      linux: none windows: cl.exe
//   clang-cl.exe clang(++)
...
// NOTE: __func__ seems to be supported everywhere (alternative in case:
// __FUNCTION__)
#define _ASSERT_STRING                                                         \
  "Assert failed for '%s' in file '%s', function: '%s' at line '%d'\n"
#define ASSERT(expr)                                                           \
  if (expr) {                                                                  \
  } else {                                                                     \
...
#define UNIMPLEMENTED                                                          \
  UNREACHABLE("Call to unimplemented function %s", __FUNCTION__)

/* TODO compile-time warning for deprecated functions. Usage: DEPRECATED int
 * my_func() { } */
#if defined(LANGUAGE_CPP)
...
#define STATIC_ASSERT(expr, msg)                                               \
  static_assert(expr, msg) /* C++11 built-in static_assert */
...
// #include <debugapi.h>
#include <windows.h>
static int debug_running_under_debugger() { return IsDebuggerPresent(); }
#else
...
#endif
#else
/* NOTE we assume we don't run under a debugger when not built with debug
 * information */
...
#undef TYPE_OF
#define TYPE_OF(e)                                                             \
  STATIC_ASSERT(0, "TYPE_OF macro doesn't work w/ MSVC in C-mode");
#endif

/* SAME_TYPE and container_of macro for all compilers (except MSVC in C-mode) */
#if defined(LANGUAGE_CPP)
#if STANDARD_VERSION >                                                         \
    2011 /* NOTE: actually works with C++11 but error with clang-cl: deduced   \
...
#define container_of(ptr, type, member)                                        \
  ...
  STATIC_ASSERT(SAME_TYPE(*(ptr), ((type *)0)->member),                        \
                "Pointer type mismatch in container_of macro")
#endif
...
/* macros to check if array is real array (and not just a pointer, i.e. decayed
 * array) */
